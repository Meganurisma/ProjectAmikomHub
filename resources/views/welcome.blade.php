    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="flex justify-between items-end mb-12">
            ...
            <div class="mb-8 flex flex-wrap gap-3 justify-center">
                <a href="/" class="px-5 py-2 rounded-full font-semibold transition duration-300 {{ request()->has('category') ? 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-100' : 'bg-indigo-600 text-white shadow-lg' }}">
                    Semua
                </a>
                @foreach($categories as $cat)
                    <a href="?category={{ $cat->slug }}" class="px-5 py-2 rounded-full font-semibold transition duration-300 border {{ request('category') == $cat->slug ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50' }}">
                        {{ $cat->name }}
                    </a>
                @endforeach
            </div>
            ...
            @foreach($events as $event)
                <div class="group bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden">
                    <div class="relative overflow-hidden bg-slate-200 h-80 rounded-t-3xl flex items-center justify-center">
                        ...
                        <div class="absolute top-4 left-4 px-3 py-1 bg-white/90 backdrop-blur rounded-full text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-600 border border-indigo-100">
                            {{ $event->category->name }}
                        </div>
                    </div>
